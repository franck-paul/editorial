...
dotclear.ready(() => {
    ...
    if (window.opener) {
        const button = document.querySelector('button[name="media-select-ok"]');

        button.addEventListener('click', () => {

            const main = window.opener;
            const href = main.document.querySelector('input[name="url"]').value;
            const thumburl = main.document.querySelectorAll('input[name="src"]')[2].value;
